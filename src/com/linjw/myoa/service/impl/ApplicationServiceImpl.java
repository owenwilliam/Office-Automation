...
public class ApplicationServiceImpl extends DaoSupportImpl<Application> implements ApplicationService{
	...
	private SimpleDateFormat adf = new SimpleDateFormat("yyyy-MM-dd");
	public void submit(Application application) {
    //1.设置属性并保存application
		
		 application.setApplyTime(new Date()); // 申请时间，当前时间
		 application.setStatus(Application.STATUS_RUNNING);
		 application.setTitle(application.getApplicationTemplate().getName()//日期格式为(模板)_(申请人姓名)_(申请时间)
				     +"_"+application.getApplicant().getName()//
				     +"_"+adf.format(application.getApplyTime()));
		...
		String pdKey = application.getApplicationTemplate().getProcessDefinition();
	...
	public List<Application> findByUser(User currentUser) {
		return getSession().createQuery(//
				"FROM Application a WHERE a.applicant=?")
	...
	public List<TaskView> getMyTaskViewList(User currentUser) {
		...
		//查询我的任务列表
		String userId = currentUser.getLoginName();//约定使用LoginName作为jbpm用户标识符
	...
	public void approve(ApproveInfo approveInfo, String taskId,String outcome) {
		...
		//2.办理完任务
		Task task = processEngine.getTaskService().getTask(taskId);//先取出Task对象，再完成任务，否则拿不到，因为执行完就变成历史信息了
		if(outcome == null){
			processEngine.getTaskService().completeTask(taskId);
		}else{
			processEngine.getTaskService().completeTask(taskId,outcome);//按照线路来办理业务
		}
		//>>取出所属的流程实例，如果取出了null，说明流程实例执行写成了，已变成了历史记录
